...
void scaleXX1CtoU81C(IplImage* imgXX1C, IplImage* imgU81C)
{
        ...
        cvConvertScale( tmp, imgU81C, 255/max_val, 0 ); //Image conversi�n with linear transformation.
}
...
/* OpenCV 2.X API wrapper
 	Function wrapped: iniFFME
*/
void FFME::init(cv::Size size, int maxNoKeyPoints)
{	
	iniFFME(size.width, size.height, 0, maxNoKeyPoints);

	// C++ OpenCV 2.x API Wrapper... some more arrays must be allocated
	int i;
	singPoints1 = (CvPoint2D32f*)cvAlloc(maxNoKeyPoints * sizeof(CvPoint2D32f));
	singPoints2 = (CvPoint2D32f*)cvAlloc(maxNoKeyPoints * sizeof(CvPoint2D32f));
	lengthDesc = m_widthArrayHist*m_widthArrayHist*m_noBinsOriHist; //Length of descriptor vector.
        descriptors1 = (float**)cvAlloc(maxNoKeyPoints * sizeof(float*));
	for(i=0;i<maxNoKeyPoints;i++)
        {
                descriptors1[i] = (float*)cvAlloc(lengthDesc * sizeof(float));
        }
        descriptors2 = (float**)cvAlloc(maxNoKeyPoints * sizeof(float*));
        for(i=0;i<maxNoKeyPoints;i++)
        {
                descriptors2[i] = (float*)cvAlloc(lengthDesc * sizeof(float));
        }
        //Correspondences.
        correspondences = (CvPoint2D32f**)cvAlloc(maxNoKeyPoints * sizeof(CvPoint2D32f*));
        for(i=0;i<maxNoKeyPoints;i++)
        {
...
void FFME::detect(cv::Mat &img, std::vector<cv::KeyPoint> &query_kpts)
{
	// Convert into IplImage (no data copy)
	if(img.channels() != 1)
		cvtColor(img, img, CV_RGB2GRAY);
	IplImage ipl_img = img;

	// Call the detection function
	singPtoDetLut(&ipl_img, singPoints1, &noSingPoints1);

	// Store singPoints1 into a vector of points...
	std::vector<Point2f> query_pts;
	for(unsigned int i=0; i<noSingPoints1; i++)
		query_pts.push_back(Point2f(singPoints1[i]));		
    ...
    {    temp[i].pt.x= query_pts[i].x;
         temp[i].pt.y= query_pts[i].y;	// ... and convert into kpts
         query_kpts.push_back(temp[i]);
         printf("size %f %f \n", query_kpts[i].pt.x, query_kpts[i].pt.y );
    }
    //printf("size %d ", (int) query_pts.size());
	   
    //KeyPoint::convert(query_pts, query_kpts);

	m_started = true;
}
void FFME::describe(cv::Mat &img, std::vector<cv::KeyPoint> &query_kpts, cv::Mat &query_desc)
{
	// Convert into IplImage (no data copy)
	IplImage ipl_img = img;

	// Call the description function
	singPtoDescLut(singPoints1, noSingPoints1, descriptors1, true);

	// Insert descriptors into cv::Mat
	cv::Mat aux(m_maxNoKeyPoints, lengthDesc, CV_32FC1, descriptors1);

	//... but last rows should be deleted
	query_desc = aux.rowRange(0, noSingPoints1);	
	
}
void FFME::match(std::vector<KeyPoint> &query_kpts, std::vector<KeyPoint> &train_kpts, std::vector<DMatch> &matches)
{
	// Call to C-function
	matchSingPtos(singPoints2, noSingPoints2, descriptors2, singPoints1, noSingPoints1, descriptors1, correspondences, &noCorr);

	// Fill matches
	// NOTE: this FFME::match function is conceptually different to the match functions of other methods of OpenCV such as
	// ORB or SIFT, since the internal C-function matchSingPtos does not work on vectors but fixed length arrays.
	// Therefore, the output of FFME::match is a re-ordered set of poitns, without distance (Hamming or NORM_L2) information
	query_kpts.clear();
	train_kpts.clear();
	matches.clear();
	for(unsigned int i=0; i<noCorr; i++)
	{	
		query_kpts.push_back(KeyPoint(Point2f(correspondences[i][1]), 0));
		train_kpts.push_back(KeyPoint(Point2f(correspondences[i][0]), 0));
		matches.push_back(cv::DMatch(i,i,0));		
	}
}

void FFME::updateBuffers()
{
	//Update buffers.
...
void FFME::singPtoDetFunc(IplImage* img_U81C, CvPoint2D32f* singPtos, int* noSingPtos)
{
        ...
        gradMagThresh(m_threshGradMag, 10); //Selection of points by thresholding of the gradient magnitude.
